...
Last updated after: Spec 0 Task 1, Spec 0 Task 2, Spec 1a, Spec 1 v3, the band-derivation follow-ups, Spec D2 Step 0, Task 787 (self-managed Clerk migration), the observability pass (Sentry + PostHog, July 28, 2026; DSNs and keys wired into the development environment the same day), the mobile visual polish pass (July 29, 2026), Task 818 (promotional Plus grants, July 29, 2026 — see the RevenueCat reconcile row in section 8), Task 824 (Free-card teaser copy refresh, July 29, 2026 — see section 5 web monetization surfaces), the build-26-lite release ops (July 29, 2026 — see the RevenueCat rows in section 8), and Task 840 (animated game card previews on the web /games hub, July 29, 2026 — see section 5).
...
| `src/components/layout/bottom-nav.tsx` | Line 25 XpCounter chrome. Below `lg`. Since Task 823 mounted ONCE by `AppShell` (outside the `PageTransition` subtree — see the P1 v2 paragraph); game subpages (outside AppShell) keep their per-page mounts. NOT on practice/chat |
