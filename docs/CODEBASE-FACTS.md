...
| C1 Gujarati sentence pilot shipped, other languages pending (July 29, 2026) | Gujarati sentence stage grown 8 to 51 per category via committed `lib/db/src/data/curatedSentencesC1.json` (43 generated entries per category, `origin:"generated_c1"`), generated offline by `artifacts/api-server/scripts/generateSentenceTopupsC1.ts` (`pnpm --filter @workspace/api-server run generate-sentences-c1`). `sentenceCount(slug, languageCode)` is language-aware (gu = 8 + C1 file length, others 8); all Gujarati seed/backfill consumers MUST use `gujaratiLessonsWithC1()`, never `GUJARATI_LESSONS` directly, or exact-count sentence validation fails. Seed inserts are chunked (50/batch) so first boot after publish cannot blow the promote health-check window. Two-check QA (blind back-translation + native-grammaticality judge, `artifacts/api-server/scripts/qaSentencesC1.ts`, frozen report `qa-c1-report.json`) ran July 29, 2026: initial 185 ok / 23 minor / 50 major; all 50 majors deleted and regenerated with the dative/experiencer grammar constraint now baked into `generateSentences` (generic rule for all languages, Gujarati few-shot examples injected only for Gujarati). Replacement pass: 38 ok / 4 minor / 7 major of 49 judged; residual majors are mostly gloss-nuance judge strictness, one dative slip. Judge CAVEAT: the grammaticality prompt names dative constructions, which produces false positives on correct nominative sentences — adjudicate flags, do not auto-trust. Minor romanization items live in the #809 backlog. Do not extend past Gujarati without user confirmation |
