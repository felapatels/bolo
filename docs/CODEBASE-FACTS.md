...
| #782 pre-existing API test failures | 15: progress/summary x1, progress/analytics x2, entitlementsGating (suite-level) x1, review ordering x3, parrot system-prompt x2, TTS cache/fallback x3, warmGreetings x2, account defaults x1 (GET /account expects dailyGoal 10 but the daily_goal column default became 50 with migration 0025; the test expectation was never updated). All reproduce on a clean tree. The `attempts` test formerly counted in this row now passes. Provenance: this doc said 15, an out-of-band tracker said 11, a July 2026 full-suite run measured 14, and a July 28 2026 rerun measured 15 (14 prior + the account-defaults failure introduced by migration 0025). Caution: the `test` workflow's captured log truncates to the tail, which can make a full-suite run look like it had only a handful of failures; count failures from a complete captured log (pnpm run test > file), never from the truncated workflow preview. Rule: this enumerated row is the single source of truth for the baseline — never track a bare count elsewhere |
