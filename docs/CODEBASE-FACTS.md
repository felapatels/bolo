...
| Stale dev-Clerk cookies poisoned prod auth (July 28, 2026) | Browsers that visited bolo-india.app during the dev-keyed window carry dev-instance JS cookies on the apex; they ride to clerk.bolo-india.app and break OAuth (err_code=authorization_invalid) and email sign-up silently. Server, DNS, Google creds, and instance config were all healthy. Remedied by a one-time versioned cookie purge in gujarati-coach src/lib/clerkCookieCleanup.ts (called from main.tsx before Clerk init; no-ops when localStorage is unavailable). Temporary: remove after the affected window ages out |
