...
**phrases** (`lib/db/src/schema/phrases.ts`): `language_code`, `category_id`, `difficulty` int, `premium` bool, `stage` text (`phrase` or `sentence`), `hint`, `sort_order`, plus Task 1 additions `accepted_answers` jsonb, `elo_difficulty`, `elo_difficulty_rd`, `exposure_count`. **No register, formality, or tag column exists.**
...
**Web:** custom `useVoiceRecorder` hook at `lib/integrations-openai-ai-react/src/audio/useVoiceRecorder.ts`. Uses `MediaRecorder` and also holds the `getUserMedia` `MediaStream` in `streamRef` with its own `AudioContext` and `AnalyserNode` computing RMS levels for silence auto-stop (~156-187). **A real amplitude signal exists but is not surfaced to callers.**

**Mobile:** `expo-audio`. `recorder.record()` and `stopAndReadRecording(recorder)`. Metering is enabled via `isMeteringEnabled: true` in `RECORDING_PRESET`. `useAudioRecorderState(recorder, 250)` exposes a **dBFS** `metering` value, already used for adaptive silence auto-stop.

> Two gotchas for anything consuming metering: the poll interval is 250ms, which is coarse for smooth visualisation, and the value is dBFS (logarithmic, mostly negative), so it must be mapped from roughly -50 to 0 onto 0 to 1 with clamping. A linear read looks dead until someone shouts.

**Existing visualisation:** web `SoundWavePulse` is a time-based loop, not audio-driven. Mobile has only a time-based pulse on the record button. Neither reflects real input.
...
| #782 pre-existing API test failures | 12: progress/summary, analytics x2, entitlementsGating, review ordering, warmGreetings, system-prompt, legacy-audio |
...
| Spec D2 v2 (speaking system) | Step 0 answered, implementation pending |
