// LAST CALL REPLACES A VOICE STOP, IT NEVER ADDS A ROW.
//
// Owner ruling, 2026-09-14 (slice 1), and the same day's clarification,
// verbatim: "essentially regardless of a game being in the middle, no 2
// neighboring stops should be the same type of lesson." @workspace/script-trace stop-play.ts is the
// one function that decides which voice rows are played as Last Call; these
// pins are the owner's own examples plus the invariant stated as a sweep, so a
// later edit that satisfies the examples but breaks the rule still fails.
//
// Written 2026-09-14 and NOT YET RUN (typecheck only while developing, per
// CLAUDE.md). Runs with the full mobile suite before the next build.
