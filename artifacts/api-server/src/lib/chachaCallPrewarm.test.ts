...
describe("warmChachaCallLines", () => {
  test("synthesizes every fixed line the call can play, on a cold cache", async () => {
    const d = deps([]);
    await warmChachaCallLines(d.impl);
    ...
    assert.deepEqual(
      ...
      ALL_KEYS.map((k) => callLineCacheKey(k)).sort(),
    );
  ...
  test("is a no-op once every clip is cached", async () => {
    const d = deps(ALL_KEYS.map((k) => callLineCacheKey(k)));
    await warmChachaCallLines(d.impl);
  ...
  test("only fills the gap when one clip is missing", async () => {
    const missing = "hello";
    const d = deps(ALL_KEYS.filter((k) => k !== missing).map((k) => callLineCacheKey(k)));
    await warmChachaCallLines(d.impl);
    assert.deepEqual(d.synthesized, [CALL_CANNED_LINES[missing].text]);
    assert.deepEqual(d.inserted.map((r) => r.cacheKey), [callLineCacheKey(missing)]);
  });
  ...
  test("one line that will not synthesize does not cost the others", async () => {
    ...
    await warmChachaCallLines(impl);
  ...
  test("call clips never collide with the chai-stall clips in the cache", async () => {
    const stall = (["greeting", "gift", "farewell"] as const).map(chachaLineCacheKey);
    for (const key of ALL_KEYS) {
      assert.ok(!stall.includes(callLineCacheKey(key)), `${key} collides with a stall clip`);
    }
  });
});
