 ...
 * @workspace/api-client-react but not listed here. Because this file IS
 * included in the web tsconfig (src/test-helpers/ is not in the exclude
 * list), a missing entry is a build error, not a silent runtime undefined.
...
export const apiClientMockDefaults = {
  ...
  getGetScenarioQueryKey: (...args: unknown[]) => ["scenario", ...args] as unknown[],
  getGetScriptTraceProgressQueryKey: () => ["script-trace-progress"] as const,
  getHealthCheckQueryKey: () => ["health-check"] as const,
  getListZoneStampsQueryKey: (...args: unknown[]) => ["zone-stamps", ...args] as unknown[],
