...
//
// // Mobile twin: bolo-mobile/__tests__/journey-intro-scroll.test.ts.

describe('the shot is capped, which is what makes further mean faster', () => {
  test('takes the same time past the cap, however much further it goes', () => {
  ...
  test('stays proportional below the cap, so a short hop is not a slow one', () => {
  ...
  test('never drops below the floor, so a nudge is still a shot', () => {
  ...
  test('reads a backwards distance as a distance', () => {
...
describe('the timings both platforms share', () => {
  ...
  test('hold long enough to actually read the zone card', () => {
...
describe('where the stop lands in the viewport', () => {
  test('sits about a third down, clamped at both ends', () => {
  ...
  test('never leads by more than a short viewport can spare', () => {
...
describe('the ease', () => {
  test('starts where it starts and ends where it ends', () => {
    expect(introScrollEase(0)).toBe(0);
    expect(introScrollEase(1)).toBe(1);
    expect(introScrollEase(0.5)).toBeCloseTo(0.5, 6);
  });

  test('accelerates away from the zone card and decelerates onto the stop', () => {
