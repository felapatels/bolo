...
import { MOMENT_ROTATION, momentRotationIndex } from "@/lib/moment-rotation";
...
//
// // Mobile twin: bolo-mobile/__tests__/moment-rotation.test.ts.

describe('the moment rotation', () => {
  test('cycles through the entries and wraps', () => {
  ...
  test('never indexes past a short list', () => {
  ...
  test('answers 0 for an empty feed instead of dividing by zero', () => {
    expect(momentRotationIndex(0, 5)).toBe(0);
  });

  test('holds long enough to be a door rather than a slot machine', () => {
