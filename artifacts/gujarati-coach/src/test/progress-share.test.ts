...
import { progressShareMessage } from "@/lib/progress-share";

/**
 * The Progress share line (build 26, the owner's option A).
 *
 * ITS TWIN IS PINNED IDENTICALLY on mobile, in
 * bolo-mobile/__tests__/progress-share.test.ts. The two builders are
...
describe("progressShareMessage", () => {
  ...
  // The button is on the screen from the first visit, so day one has a line.
