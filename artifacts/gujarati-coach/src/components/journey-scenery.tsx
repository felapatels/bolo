...
export const STALL_PLACEMENT = {
   ...
   *  it needed a row of its own and why HALT_H had to grow from 74 to 96 on
   *  2026-08-25 when a card's second line reached it. The left of a left-flank
   *  marker is empty.
   *
...
export function planZoneScenery(
  ...
  // cow in them, so most of the line had none: reported as "there's supposed to
  // be a roaming cow in each zone, but don't see it".
  //
  // Substituted into the LAST slot rather than appended, so the count and the
  // rows are untouched and nothing about the layout, the spacing rules or the
  // rail-clearance geometry moves. The zone's primary character (theme[0])
  // always survives; what it costs is the third element in a big zone, or the
  // only element in a zone of fewer than six stations.
  //
  // Not absolute: a cow whose row hosts Chacha-ji's stall is still dropped by
  // the caller, which keeps that strip for him.
...
export function planZoneSignpost(
  zoneIndex: number,
  stationCount: number,
  avoidRows: ReadonlySet<number> = new Set(),
): { row: number } | null {
  if (stationCount <= 0) return null;
  const taken = new Set(planZoneScenery(zoneIndex, stationCount).map((s) => s.row));
  // Rows hosting a Chacha-ji stall are taken too: the stall's lane clips the
  // signpost's board at the same ground line.
  for (const row of avoidRows) taken.add(row);
...
export function SceneryElement({
  ...
  return (
    <g
      ...
      data-testid={testId ?? "scenery-item"}
      data-scenery={kind}
