...
function taught(base: number, prop: string, vowels: boolean, skip?: Set<number>): Char[] {
  ...
  for (let off = lo; off <= hi; off++) {
    ...
    if (!re.test(char) || !isLetter.test(char)) continue;
...
for (const [prefix, base, prop] of TARGETS) {
  for (const kind of ["vowels", "consonants"] as const) {
    ...
    const wanted = taught(base, prop, kind === "vowels", SKIP_CONSONANT_OFFSETS[prefix]);
    const blocks: string[] = [];
    for (const w of wanted) {
      ...
      const off = w.char.codePointAt(0)! - base;
      const id = `${ID_PREFIX[prefix]}_${NAMES[off][0]}`;
