...
- **Art work: produce the artifact and show it, same as the audio rule.** Render the images and put them in the reply. Do not audit your own output against a second method to prove it is clean, and do not write the judgement yourself: coverage, hem, shoulders, how it reads, and whether it beats what ships are all the user's to make by eye. If a file is too large to preview, say so and supply a resized version, never an analysis in its place. ONE exception, which the eye cannot catch: report defects that are invisible at preview scale but visible after compositing, as a single number with no methodology (for garments, the count of surviving magenta-ish pixels after keying, since a 1-2px fringe composites as a coloured halo on Bolo's teal). Requested August 12, 2026.
- **Tests: run the minimum set that covers the change, never a full suite by default.** Scope is the changed files plus anything importing them. Typecheck always, it is cheap. Report exactly which tests were run and why that set. Full api-server, web, or mobile suites only on explicit request, or for the two exceptions below. Rationale: the full run takes minutes behind an advisory lock and keeps producing starvation timeouts that get diagnosed instead of read, whereas a targeted run returns in seconds and actually gets read. EXCEPTIONS, where a defect reaches production data or real money, so run the relevant full suite anyway: any change adding or altering a database migration, and any change to tokenService, the token ledger, the RevenueCat or Stripe webhook paths, or entitlement resolution. Never run the api and web suites concurrently, that alone produces timeouts unrelated to the change. Requested August 13, 2026.
