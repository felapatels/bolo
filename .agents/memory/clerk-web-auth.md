...
- Remedy v2: detect-then-purge on the prod host before Clerk init (src/lib/clerkCookieCleanup.ts). Purge fires ONLY when a dev-era distinguisher is visible in document.cookie: `__client` (prod's is HttpOnly on clerk.<domain>, invisible on the apex) or `__clerk_db_jwt*` (dev instances only). `__session`/`__client_uat` are legit prod cookies — never triggers, only cleared as part of a triggered purge.
- LESSON: never gate a destructive per-load action on localStorage — privacy extensions make setItem "succeed" without persisting, so a v1 localStorage marker looped users out after EVERY sign-in (incognito fine, extension browser broken). Use a first-party cookie marker (Domain=.<apex>, Secure, SameSite=Lax) and write+read-back it BEFORE deleting anything as a write-probe; abort if it doesn't stick. Remove the whole cleanup after the affected window ages out.
