...
**Spread gate (anti-tap):** score also multiplies by `min(1, drawnBBoxDiag / (0.45 × refBBoxDiag))`. **Why:** a stationary tap concentrates all ink in one spot yet sat within tolerance of enough reference points to pass on compact/word-scale glyphs (3 items were tap-passable). Honest traces span nearly the full glyph → factor ≈ 1; taps are crushed. Kept identical on both platforms.
...
**Animation (pen-stroke skeleton):** never animate the glyph *outline* — it is the perimeter of a filled shape, so tracing it draws around the outside of the letter, not how it is written. The demo extracts real pen strokes by skeletonizing the glyph (Tegaki-style pipeline): rasterize interior to a res×res bitmap (**adaptive res by subpath count: 64 letters / 96 words ≥8 / 128 sentences ≥20** — at 64, multi-line sentences collapse below thinning resolution and honest coverage suffers) → Zhang-Suen thinning → trace 1-px skeleton into polylines split at junctions → **merge collinear segments across junctions BEFORE spur-pruning** (the tiny junction fragments are the bridges; pruning first leaves gaps too wide to merge) → prune spurs → **loop-safe RDP** (`simplifyStroke`: plain RDP's start≈end chord is degenerate on closed loops, measuring every point at distance ~0 and collapsing rings to zero-length segments; split at the farthest-from-start point and RDP each half) → **Chaikin corner-cut ×2** (RDP output is angular; Chaikin rounds it into hand-drawn curves, stays inside the point hull so interior tests keep passing, endpoints preserved so start dot/ordering unaffected) → orient strokes top-left-first → greedy nearest-next ordering. Result: ~3 natural strokes for simple glyphs (headline, stem, curve). **Hairline fallback:** sentence-scale glyphs squeeze limbs thinner than one raster cell; point-sampled winding shatters that ink into isolated pixels the tracer drops (skeleton collapses to fragments). When skeleton totalLen < 20 or < 0.08× outline length, use the outline loops directly as pen strokes — outline ≈ centreline for hairline glyphs. Do NOT "fix" this by dilating the raster instead: dilation bridges real sub-cell gaps in dense scripts and merges limbs into blobs with short wrong skeletons. **Perf:** strokes cached in a module-level Map keyed by guide string (`strokesForGuide`) — `useMemo` alone recomputes per remount/retry, and 96/128-res extraction costs up to a few hundred ms on mobile JS thread. Demo duration scales with stroke count (`clamp(strokes/6, 1, 3) × base`). The green start dot = first point of the first pen stroke. Rejected approaches: outline dashoffset, scanline sweeps, boustrophedon interior dot — all rejected by user as "not how you write".
