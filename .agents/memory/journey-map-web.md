...
**Cutout ruling (approved July 30, 2026):** punch/notch circles on ticket surfaces may only straddle card edges as bites (perforation ends, edge midpoints) — never float inside the card. Web's floating PunchHole and the journey header's notch dot + 🎫 emoji were deliberately dropped from the mobile port (emoji = tofu without an emoji font); don't re-copy them in future ports. `qa/notch-probe.mjs` measures circle rects and flags FULLY-INSIDE vs STRADDLES-EDGE.
