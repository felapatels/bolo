...
- **EAS production env must include `EXPO_PUBLIC_CLERK_PROXY_URL=https://<prod-domain>/api/__clerk`** (alongside DOMAIN + pk_live key). Without it the store binary hits `clerk.<domain>` directly, which has no working TLS → Clerk never loads → blank white screen behind `<ClerkLoaded>` at launch.
