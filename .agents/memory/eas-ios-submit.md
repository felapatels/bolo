...
- Submission status ERRORED can be a FALSE NEGATIVE: with `error: null` + `logsUrl: null` in GraphQL and wait-mode CLI showing only "Something went wrong when submitting your app to Apple App Store Connect", the binary may still have reached TestFlight (July 29, 2026: all three build-26 submissions showed ERRORED, yet the build was up in TestFlight). Before retrying or escalating, have the owner check TestFlight — later "failures" may just be duplicate-binary rejections. No error detail is retrievable with EXPO_TOKEN alone.
...
- **Upload protocol (owner ruling, July 30, 2026): do NOT run `eas submit`.** It failed six times with null errors on build 26; since build 27 the owner uploads the .ipa via Transporter themselves. After a build finishes, provide the Application Archive URL from `eas build:view` and stop.
